...
const BookingResultTimeSlotList = ({empty, slot = {}, selectedCourts = [], onCourtSelect, duration}) => {
  ...
  const start_time = parsedTime.format('HH:mm');
  const courts = !!Object.keys(slot).length && slot.available_courts.map(court => ({...court, start_time})) || [];
  const remainingCourts = courts.filter(court =>
    !selectedCourts.some(selected => selected.id === court.id && validTime(selected.start_time, court.start_time, duration)));
