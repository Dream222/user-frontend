...
const ACTION_HANDLERS = {
  ...
  ON_CANCEL_RESERVATION_SUCCESS: (state, action) => {
    const index = state.content.reservations.indexOf(action.payload)
    ...
    return {
      ...
      content: {
        ...state.content,
        reservations: [
          ...state.content.reservations.slice(0, index),
          ...state.content.reservations.slice(index + 1)
        ]
      }
    }
  },
  ON_RESELL_RESERVATION_SUCCESS: (state, action) => {
    ...
    return {
      ...
      content: {
        ...
        reservations: [
          ...state.content.reservations.slice(0, index),
          Object.assign({}, action.payload,{ isReselling: !action.payload.isReselling }),
          ...state.content.reservations.slice(index + 1)
        ]
      }
    }
