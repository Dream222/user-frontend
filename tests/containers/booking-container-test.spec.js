...
describe('Booking modal', () => {
  ...
  describe('state provided by the store', () => {
    it('passes down props correctly', () => {
      expect(app.props.icons).to.deep.equal(initialState.venues.allSports);
      expect(app.props.sport).to.deep.equal(initialState.location.query.sport_name);
      expect(app.props.chosenVenue).to.deep.equal(initialState.search.searchResults[1]);
      ...
      expect(app.props.slots.length).to.equal(Object.keys(initialState.search.searchResults[1].data.time_slots).length);
