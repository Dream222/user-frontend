...
describe('(Redux Module) Profile', () => {
  ...
  describe('reducer', () => {
    it('should remove reservation on ON_CANCEL_RESERVATION_SUCCESS', () => {
      ...
      expect(profileReducer(state, { type: 'ON_CANCEL_RESERVATION_SUCCESS', payload: state.content.reservations[1] }))
      .to.deep.equal({
        content: {
          reservations: [
            { id: 3303, booking_type: 'online' },
    ...
    it('should change isReselling on ON_RESELL_RESERVATION_SUCCESS', () => {
      const state = {
        content: {
          reservations: [
            { id: 3303, isReselling: false },
            { id: 3345, isReselling: false },
            { id: 3347, isReselling: false }
          ]
        }
      };
      expect(profileReducer(state, { type: 'ON_RESELL_RESERVATION_SUCCESS', payload: state.content.reservations[1] }))
        .to.deep.equal({
          content: {
            reservations: [
            { id: 3303, isReselling: false },
            { id: 3345, isReselling: true },
            { id: 3347, isReselling: false }
            ]
          }
        });
