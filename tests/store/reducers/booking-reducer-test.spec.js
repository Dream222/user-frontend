...
describe('reducer', () => {
  ...
  describe('on TOGGLE_LOADED action', () => {
    ...
    it('returns the state with chosen slot', () => {
      expect(reducer(state, {type: 'DISPLAY_BOOKING_RESULTS'})).to.deep.equal({displayBookingResults: true});
    });
  });
  ...
  describe('on SET_AVAILABILITY_VIEW action', () => {
    ...
    it('returns the state with selected duration', () => {
      expect(reducer(state, { type: 'SET_AVAILABILITY_VIEW', payload: 'compact' })).to.deep.equal({availabilityView: 'compact'})
    })
  })
  ...
  describe('on ADD_GAME_PASS action', () => {
    ...
    it('applies game pass to all related courts', () => {
      expect(reducer(state, {type: 'ADD_GAME_PASS', courtId: 1, payload: payload})).to.deep.equal({
        selectedCourts: [
